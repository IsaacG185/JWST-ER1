...
dir = "data/jw01727-o138_t096_nircam_clear-f115w/"  # directory containing your files

i2d_file  = dir + "jw01727-o138_t096_nircam_clear-f115w_i2d.fits"     # your _i2d file
segm_file = dir + "jw01727-o138_t096_nircam_clear-f115w_segm.fits"    # matching _segm file
cat_file  = dir + "jw01727-o138_t096_nircam_clear-f115w_cat.ecsv"     # matching _cat file
# Approximate sky position of the Einstein ring (if you know it)
# If you don't know RA/Dec yet, you can skip this block
have_ring_coords = False
ring_ra  = 0.0   # degrees
ring_dec = 0.0   # degrees
...
# Show the i2d image
norm = simple_norm(sci_data, 'sqrt', clip=True)
img = ax.imshow(sci_data, origin='lower', norm=norm, cmap='gray')
...
if have_ring_coords:
    ring_x, ring_y = wcs.world_to_pixel_values(ring_ra, ring_dec)
    ax.plot(ring_x, ring_y, marker='+', color='red',
            markersize=12, mew=2,
            transform=ax.get_transform('pixel'), label='Einstein ring')

    # Look up segmentation label at that pixel
